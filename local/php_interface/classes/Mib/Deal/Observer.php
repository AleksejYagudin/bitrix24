...
class Observer
{
    public static function handleOnBeforeCrmDealUpdate(array &$field): bool {
        ...
        if (empty($lastComment) || $lastComment['AUTHOR_ID'] != $deal->getId()) {
            ...
            $field['RESULT_MESSAGE'] = '1Чтобы перевести сделку в неуспешный статус, необходимо, чтобы ответственный оставил комментарий. Внесённые изменения не будут применены.';
            self::sendNotify();
            return false;
        }
    ...
    private static function sendNotify(): void {
        ...
        \CIMNotify::Add([
            ...
            'MESSAGE'    => '2Чтобы перевести сделку в неуспешный статус, необходимо, чтобы ответственный оставил комментарий. Внесённые изменения не будут применены.'
        ]);
    }
}